 ...
 */
    //swagger doc says, while sending spartan post request id is created automatically

    //way 1- FOR SENDING REQUEST PART; We save json body in a string
    @Test
    public void PostNewSpartan() {
        ...
        assertEquals(response.jsonPath().getLong("phone"), 8877445596l);

    }

    //way 2- FOR SENDING REQUEST PART; We save info in a map structure(java)
    //then serialize the java in body() method to json so that we can post /send to the API
    @Test
    public void PostNewSpartan2() {
        ...
        given().log().all()
                ...
                .contentType(ContentType.JSON)
                .and()
                .body(requestMap)                                      //here in body, our map automaticly is converted to json
                .when()
                .post("/api/spartans")
                .then().log().all()                                    //WE used hamcrestMatcher for verifying
                .statusCode(201)
                .and()
                .contentType("application/json")
                .and()                                                  //equalTo or is both are correct
    ...
    //way 3- FOR SENDING REQUEST PART We save info in a class then put in body method to serialize/convert to json
    //we used spartan respond class for post(it is not proper :))
    //but normally our post pojo class must be different class (we did proper one for HrPost request with regionPost class)
    //Look at the region post for this we created new class it has to be new class
    @Test
    ...
    public void PostNewSpartan4() {
        ...
        // here for each sending post id changes dynamically, we saved post request and for having id in the response part
    ...
    //(This hw can be done thru-> PostNewSpartan3())
